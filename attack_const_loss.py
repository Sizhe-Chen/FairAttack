...
def attack(net_name, optimizer_name, start_id, end_id, gpu_id):
    ...
    # record directory
    result_dir['base'] = f'{get_time()}_{net_name}_{optimizer_name}_CSTloss_Iter{iter_thres}_RMSD{rmsd_first_thres}_Eps{epsilon}_Index{start_id}to{end_id}_GPU{gpu_id}'
    ...
    for sam_id, file in enumerate(file_lists):
        ...
        if iter_done < iter_thres:
            ...
            loss_value = sess.run(loss, {inputs['image']: [sample['max_loss']], inputs['label']: [prob['label']]})
